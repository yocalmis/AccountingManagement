...
class Yonetim extends CI_Controller
{
   ...
   /*     $deneme_adet = $this->session->userdata($username . '_giris_adedi');
        $this->session->set_userdata('deneme_username', $username);
        if ($deneme_adet >= 3) {

            $this->session->set_userdata("bitis", date('Y-m-d h:i:s'));
            $bitis = strtotime($this->session->userdata("bitis"));
            $bas   = strtotime($this->session->userdata("baslangic"));
            $dk    = round(abs($bitis - $bas) / 60, 0);

            if ($dk <= 15) {
                $this->load->model('admin_model');
                $return = $this->admin_model->uye_Varmi($username);

                if ($return == 1) {
                    $this->session->set_userdata($username . "giris_deneme_hatasi", 1);
                    $this->load->library('Messages');
                    echo $this->messages->config('yonetim');
                    return false;
                }
                $this->session->set_userdata($username . "giris_deneme_hatasi", 0);
                $this->load->library('Messages');
                echo $this->messages->config('yonetim');
                return false;
...
/**/
    ...
    public function cari($edit = null, $id = null)
    {
        ...
        if ($this->login_kontrol($online) == true) {
            ...
            $crud->columns('unvan', 'kisi_turu', 'Cari İşlemleri', 'Cari Kaldır');
            $crud->where('kullanici_id', $this->session->userdata('kullanici_id'));
            $crud->where('kisi_turu!=', 1);
            $crud->required_fields('adi_soyadi', 'unvan', 'tc', 'eposta', 'kisi_turu');
    ...
    public function potansiyel($edit = null, $id = null)
    {
        ...
        if ($this->login_kontrol($online) == true) {
            ...
            $crud->columns('unvan', 'kisi_turu', 'Potansiyel Taşı');
            $crud->where('kullanici_id', $this->session->userdata('kullanici_id'));
            $crud->where('kisi_turu!=', 1);
            $crud->required_fields('adi_soyadi', 'unvan', 'tc', 'eposta', 'kisi_turu');
    ...
    public function hizmet_urun($edit = null, $id = null)
    {
        ...
        if ($this->login_kontrol($online) == true) {
            ...
            $crud->columns('adi', 'alis_fiyat', 'satis_fiyat', 'Ürün Kaldır');
            $crud->where('kullanici_id', $this->session->userdata('kullanici_id'));
    ...
    public function uyeler($edit = null, $id = null)
    {
        ...
        if ($this->login_kontrol($online) == true) {
            ...
            if ($state == 'add') {
                ...
                $crud->set_relation('cari_id', 'cari', 'adi_soyadi', ['kullanici_id = ' => $this->session->userdata('kullanici_id'), "kisi_turu" => 1]);
            ...
            $crud->field_type('yetki', 'dropdown',
                array('0' => 'Full Yetki', '1' => 'Düzenle', '2' => 'Görüntüle'));
...
//   $this->log("Görev Silme","task","Silme",0);
    ...
    public function fatura($edit = null, $id = null)
    {
        ...
        if ($this->login_kontrol($online) == true) {
            ...
            $crud->columns('fatura_turu', 'fatura_no', 'cari_id', 'toplam', 'vade_tarihi', 'düzenle', 'görüntüle', 'personel', 'Fatura Kaldır');
            $crud->display_as('cari_id', 'Cari Adı');
            $crud->where('kullanici_id', $this->session->userdata('kullanici_id'));
            $crud->field_type('kullanici_id', 'hidden', $this->session->userdata('kullanici_id'));
            ...
            $crud->callback_column('Fatura Kaldır', array($this, 'callback_fat_kaldir'));
    ...
    public function callback_fat_kaldir($value, $row)
    {
        ...
        if ($this->login_kontrol($online) == true) {
            ...
            return "<a class='btn btn-default' href='" . site_url('yonetim/fatura_delete/' . $row->id) . "'>Fatura Kaldır</a>";
        }
...
//print_r($data["irsaliye"]);
    ...
    public function fatura_al()
    {
        ...
        if (empty($online)) {
            ...
            $fat_kayit = $this->admin_model->fat_kayit($fat_turu, $ara_toplam, $indirim, $vergi, $toplam, $mus, $seri, $no, $duz_ta, $va_ta, $ack, $irs_durum, $fat_turu, $this->session->userdata('kullanici_id'), $per);
            $fat_id    = $fat_kayit;
    ...
    public function iade_fatura_al()
    {
        ...
        if (empty($online)) {
            ...
            $fat_kayit = $this->admin_model->iade_fat_kayit($fat_id, $fat_turu, $ara_toplam, $indirim, $vergi, $toplam, $mus, $seri, $no, $duz_ta, $va_ta, $ack, $irs_durum, $fat_turu, $this->session->userdata('kullanici_id'), $per);
            $fat_id    = $fat_kayit;
 ...
 */
    ...
    public function fatura_guncelle()
    {
        ...
        if (empty($online)) {
            ...
            $islem_kayit = $this->admin_model->islem_gunc_fat($fat_id, "4", "Fatura", $fat_id, $gircik, $toplam, $duz_ta, $ack, $mus, $this->session->userdata('kullanici_id'));
...
//daha önce tahsilat kaydı yapılmış şu an güncellenecek
    ...
    public function siparis_al()
    {
        ...
        if (empty($online)) {
            ...
            $fat_kayit = $this->admin_model->sip_kayit($fat_turu, $ara_toplam, $indirim, $vergi, $toplam, $mus, $seri, $no, $duz_ta, $va_ta, $ack, $irs_durum, $fat_turu, $this->session->userdata('kullanici_id'));
            $fat_id    = $fat_kayit;
 ...
 */
    ...
    public function teklif_al()
    {
        ...
        if (empty($online)) {
            ...
            $fat_kayit = $this->admin_model->tek_kayit($fat_turu, $ara_toplam, $indirim, $vergi, $toplam, $mus, $seri, $no, $duz_ta, $va_ta, $ack, $irs_durum, $fat_turu, $this->session->userdata('kullanici_id'));
            $fat_id    = $fat_kayit;
